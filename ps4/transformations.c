   ...
   //malloc 
	...
	for (uint32_t i = 0; i < height; i++){
		for(uint32_t j = 0;j<width;j++){
			pixely[(i*width)+j].red=image -> data[(width-y-1)*width].red;
			pixely[(i*width)+j].blue=image -> data[(width-y-1)*width].blue;
            pixely[(i*width)+j].green=image -> data[(width-y-1)*width].green;
